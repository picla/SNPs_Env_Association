...
G.output.h5 <- '/groups/nordborg/user/pieter.clauw/Documents/Source/genotypes_for_pygwas/1.0.0/1001genomes_R_compatible/all_chromosomes_binary_gzip_attrs_GxE.hdf5'
```


```{r select genes}
gene.selection <- filter(gene_cat, category == 'GxT') %>%
  pull(genes)
...
h5write(accessions, G.output.h5, '/accessions', write.attributes = T)
...
positions.new <- unlist(lapply(colnames(snp.selection), function(x){as.numeric(strsplit(x, '_')[[1]][2])}))

attr(positions.new, 'chrs') <- h5readAttributes(G.file, '/positions')$chrs
attr(positions.new, 'chr_regions') <- chr_regions.new
h5write(positions.new, G.output.h5, '/positions', write.attributes = T)
...
h5write(snps, G.output.h5, '/snps', write.attributes = T)
h5closeAll()
```
